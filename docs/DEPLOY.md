# デプロイ手順（会議ルーター / 社内版「会議関所」）

会議ルーターは、単一の静的ページ（`index.html`）と、AIアシスト用のステートレスAPI（`server/router-api.mjs`）で構成されます。サーバーは npm 依存ゼロ・Node 標準ライブラリのみで動作し、**ユーザー本文を保存・記録しない（ゼロリテンション）** 設計です。

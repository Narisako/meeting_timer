...
AIアシストは全ルート共通で単一モデル（環境変数 `OPENAI_MODEL`／既定 `gpt-4o`）を使用します。宛先は `OPENAI_BASE_URL` で指定した OpenAI 互換 API です。

| 用途 | モデル | 考え方 |
|------|--------|--------|
| 対話インテーク・R1〜R7 の全ルート | `OPENAI_MODEL`（既定 `gpt-4o`） | 用途ごとの切り替えはせず、運用者が選んだ1モデルで統一。コストや品質に応じて環境変数で差し替える。 |
